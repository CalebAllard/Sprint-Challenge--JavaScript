...
// graduates.map((element) =>  element.university);
...
const unisWithUni = graduates.filter((element) => { if (element.university.includes('Uni')){ return element; } });
// graduates.filter((element) => { if (element.university.includes('uni')){return element;} });
...
// zooAnimals.map(element => {return `${element.animal_name}`}).toLowerCase();
...
*/
const lowPopulationAnimals = zooAnimals.filter(element => { if (element.population < 5) return element});
...
// zooAnimals.reduce((acc, curr) =>{ return acc + curr; },0);
